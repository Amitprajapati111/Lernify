                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {materials.map(mat => (
                                            <div key={mat._id} className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md transition group">
                                                ...
                                                {mat.fileUrl && (
                                                    <a href={`${import.meta.env.VITE_API_BASE_URL ? import.meta.env.VITE_API_BASE_URL.replace('/api', '') : 'http://localhost:5000'}${mat.fileUrl}`} target="_blank" rel="noreferrer" className="text-sm font-medium text-primary-600 hover:text-primary-700 mt-4 inline-block">View File &rarr;</a>
                                                )}
                                            </div>
                                        ))}
                                    </div>
                                        ...
                                        {assessments.map(task => (
                                            <div key={task._id} className="bg-white border border-slate-200 rounded-xl p-5 hover:border-slate-300 transition flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                                                ...
                                                <div className="text-right shrink-0">
                                                    ...
                                                    {task.fileUrl && (
                                                        <a href={`${import.meta.env.VITE_API_BASE_URL ? import.meta.env.VITE_API_BASE_URL.replace('/api', '') : 'http://localhost:5000'}${task.fileUrl}`} target="_blank" rel="noreferrer" className="inline-flex items-center text-sm text-primary-600 hover:text-primary-700 font-medium mt-2">
